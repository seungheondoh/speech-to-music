...
class HIKIA_Dataset(Dataset):
    def __init__(self, split):
        self.split = split
        self.tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
        self.processor = Wav2Vec2Processor.from_pretrained('facebook/wav2vec2-base-960h')
        self.maxseqlen = SPEECH_SAMPLE_RATE * 1
        if split == 'TRAIN':
            self.fl = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/train.csv"), index_col=0)
        elif split == 'VALID':
            self.fl = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/valid.csv"), index_col=0)
        elif split == 'TEST':
            self.fl = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/test.csv"), index_col=0)
        elif split == "ALL":
            tr = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/train.csv"), index_col=0)
            va = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/valid.csv"), index_col=0)
            te = pd.read_csv(os.path.join(CLSREG_DATASET, f"split/HIKIA/test.csv"), index_col=0)
    ...
    def __getitem__(self, index):
        ...
        binary = item.values
        text = "hi,kia"
        audio = np.load(os.path.join(CLSREG_DATASET, f"feature/HIKIA/npy/{fname}.npy"), mmap_mode='r')
