...
parser.add_argument('--framework', type=str, default="2branch")
...
def main_worker(args):
    ...
    save_dir = f"exp/{args.audio_type}_{args.speech_type}/{args.modality}_{args.framework}"
...
def train(train_loader, model, optimizer, epoch, logger, args):
    ...
    for data_iter_step, batch in enumerate(train_loader):
        ...
        # compute output
        loss, _ = model(s_emb, s_tag_emb, s_va, s_binary, m_emb, m_tag_emb, m_va, m_binary)
...
def validate(val_loader, model, epoch, args):
    ...
    model.eval()
    epoch_end_loss, epoch_end_stm_loss = [], []
    for data_iter_step, batch in enumerate(val_loader):
        ...
        m_binary = batch["m_binary"].cuda(args.gpu, non_blocking=True)
        # compute output
        loss, stm_loss = model(s_emb, s_tag_emb, s_va, s_binary, m_emb, m_tag_emb, m_va, m_binary)
